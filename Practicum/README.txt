...
. - Существует 4 случая при конкатенации, при реализации объедены в 2 случая:
      x^n.x^m
      x^n u + x^m v
      x^n u + x^m
      x^n + x^m v

* - всевозможные префикс-слова из префикс-слов, 
...
На каждый символ не более k^4 итерации => асимптотика O(k^4 * |expression|).
